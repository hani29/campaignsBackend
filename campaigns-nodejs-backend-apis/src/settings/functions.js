...
const setIntercomAttributes = (req, attributes) => {
  if(attributes && attributes.email){
    // Find user by email
    req.intercomClient.users.find({ email: attributes.email }, (res) => {
      if(res.body && res.body.users.length){
        // Update a user
        req.intercomClient.users.update({...attributes});
      }else{
        // Create a user
        attributes.signed_up_at = new Date();
        req.intercomClient.users.create({...attributes});
      }
    });
  }
}
